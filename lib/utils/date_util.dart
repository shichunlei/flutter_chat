...
const int DAY_MILLISECONDS = DAY_SECONDS * 1000;
...
/// 今天的日期
///
String today({List<String> formats}) {
  if (null == formats) {
    formats = [yyyy, '-', mm, '-', dd];
  }
...
/// 给定日期是否是今天
///
bool isToday(String dateStr, {List<String> formats}) {
  if (null == formats) {
    formats = [yyyy, '-', mm, '-', dd];
  }
...
/// [datetime]
///
String friendlyDateTime(String datetime, {bool isLocal: true}) {
  String friendly = '';
  ...
  } else if (weeks >= 1 && mounts < 1) {
    friendly = '$weeks周$agoOrAfter';
  } else if (mounts >= 1 && mounts < 6) {
    friendly = '$mounts月$agoOrAfter';
  } else if (mounts >= 6 && mounts < 12) {
